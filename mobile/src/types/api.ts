...
// shape) - every paginated backend endpoint returns this.
...
// response from the backend, regardless of which layer produced it.
// `code` is the stable, machine-readable identifier to branch on - never
// `message` (human-readable, may change) or `status` alone (several codes
// can share the same HTTP status).
